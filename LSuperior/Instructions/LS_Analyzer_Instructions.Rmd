...
The number of days sampled, the mean pressure count ($\bar{f}_{ij}$), and the variance of the mean pressure count ($V(\bar{f}_{ij})$) was then computed across the *observed* days for each month ($i$) and daytype ($j$) combination. The $\bar{f}_{ij}$ were then expanded to be the **total for each month-daytype** by multiplying by the number of available days ($N\_f_{ij}$) of that daytype within that month; i.e., $\tilde{f}_{ij}=\bar{f}_{ij}\times N\_f_{ij}$. The variances of totals for each month-daytype were computed as $V(\tilde{f}_{ij})=V(\bar{f}_{ij})\times (N\_f_{ij})^2$. Finally, the total pressure count was summarized by months across daytypes (i.e., $\tilde{f}_{i\cdot}$ and $V(\tilde{f}_{i\cdot})$), by daytypes across months (i.e., $\tilde{f}_{\cdot j}$ and $V(\tilde{f}_{\cdot j})$), and for the entire year (across month and daytype; i.e., $\tilde{f}_{\cdot \cdot}$ and $V(\tilde{f}_{\cdot \cdot})$).
...
A new variable called INDHRS is created that is the product of PERSONS and HOURS and, thus, represents the number of hours of fishing by individuals (rather than by the party).

These "corrected" raw data are then summarized by month to compute the total number of interviews (NINTS), total interviewed effort (hours) of ALL parties (HOURS), mean interviewed effort (in hours) by COMPLETED parties (MTRIP), mean person's per party (PARTY), and the proportion of total interviewed effort for month-daytype that is in a given waters-fishery (PROP). Additionally, the uncorrected sum-of-squares for the total interviewed effort is computed and is ultimately used later in variance calculations.
